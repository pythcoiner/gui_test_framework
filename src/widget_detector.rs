...
fn color_arg(map: &ItemMap) -> String {
    ...
    let color_list = map.keys().collect::<Vec<_>>();
...
fn detect(colors: &str, path: &Path) -> Result<String, DetectError> {
    println!("dw -f {} -c {} ", path.to_str().unwrap(), colors);
    ...
    if dw.status.success() {
        let out = String::from_utf8(dw.stdout).map_err(|_| DetectError::ParseUtf8);

        println!("out={}", out.clone().unwrap());
...
pub fn detect_items(map: &ItemMap, frame: &RgbaImage) -> Result<Vec<Item>, DetectError> {
    ...
    // get current dir
    let mut dir = std::env::current_dir().expect("Should no fail getting cwd!");
    dir.push("frame.png");
    ...
    let out: Response = serde_json::from_str(&r).map_err(|_| DetectError::ParseResponse)?;
